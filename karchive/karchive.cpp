...
KArchiveDirectory *KArchivePrivate::findOrCreate(const QString &path, int recursionCounter)
{
    ...
    if (recursionCounter > 2500) {
        qWarning() << "path recursion limit exceeded, bailing out";
        return nullptr;
    }
    //qDebug() << path;
    ...
    if (existingEntry) {
            ...
            //qDebug() << "found it";
        {
            ...
            if (file->size() > 0) {
                qWarning() << path << "is normal file, but there are file paths in the archive assuming it is a directory, bailing out";
                return nullptr;
            }

            qDebug() << path << " is an empty file, assuming it is actually a directory and replacing";
